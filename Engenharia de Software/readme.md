...
🎯 A obra foca em três princípios fundamentais:
⏳ Tempo e Mudança
...
Como a organização deve se adaptar à medida que cresce e evolui.

💰 Compromissos e Custos

Como as decisões devem ser tomadas levando em conta o tempo, as mudanças e a escala de crescimento da organização.
...
Refere-se ao percentual de tempo em que o sistema deve estar funcionando e acessível.
...
É a capacidade do sistema de continuar funcionando bem quando o número de usuários ou requisições aumenta.

Exemplo: um site de leilão online que precisa suportar milhares de usuários simultâneos sem travar.

⚡ Desempenho (Performance)

Relaciona-se ao tempo de resposta e à eficiência do sistema no uso de recursos (CPU, memória, rede).

Exemplo: uma página que deve carregar em até 2 segundos mesmo com muitos acessos.

🔒 Segurança (Security)

Garante proteção contra acessos indevidos, ataques e vazamento de informações.
...
⚖️ 4 - Trade-offs (negociação entre requisitos não funcionais)
...
A Google implementou a autenticação em dois fatores (2FA) e, mais recentemente, o uso de chaves de acesso (passkeys) para aumentar a segurança das contas dos usuários. Essas medidas reduzem significativamente o risco de invasões e roubos de credenciais. Porém, muitos usuários consideram o processo de login mais demorado, já que precisam confirmar a identidade por meio de um celular, aplicativo autenticador ou chave de segurança.

Trade-off: maior proteção contra ataques cibernéticos, mas menor praticidade e rapidez no acesso.

⚡ Cenário 2 – Desempenho vs. Custo
Exemplo: Netflix

A Netflix investe bilhões de dólares em infraestrutura de nuvem e em sua rede própria de distribuição de conteúdo (CDN) para garantir que filmes e séries sejam transmitidos com alta velocidade e qualidade para milhões de usuários ao redor do mundo. Isso proporciona excelente desempenho e reduz interrupções no serviço.

Trade-off: melhor experiência para o usuário e maior desempenho do sistema, porém com custos muito elevados de infraestrutura e operação.

📈 Cenário 3 – Escalabilidade vs. Complexidade
Exemplo: Amazon

A Amazon substituiu parte de sua arquitetura monolítica por uma arquitetura baseada em microsserviços para suportar o crescimento da plataforma e milhões de acessos simultâneos. Essa abordagem permite que diferentes partes do sistema cresçam de forma independente e escalável.

No entanto, a arquitetura se tornou muito mais complexa, exigindo monitoramento avançado, integração entre serviços e equipes especializadas para manutenção.

Trade-off: alta capacidade de crescimento e escalabilidade, mas maior complexidade de desenvolvimento e manutenção.
